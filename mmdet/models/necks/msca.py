...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, 
        ...
        #
        self.pool1 = nn.AvgPool2d(ds_ratio, stride=ds_ratio)
        self.pool2 = nn.AvgPool2d(ds_ratio//2, stride=ds_ratio//2)
    ...
    def forward(self, x1, x2, H, W):
        ...
        assert N == H*W
        q = self.q(x1).reshape(B, N, self.num_heads, C // self.num_heads)
        q = q.permute(0, 2, 1, 3).contiguous()
        #
        x_ = N2WH(x2, H, W) 
        r1 = self.pool1(x_).reshape(B, C, -1).permute(0,2,1)
        r2 = self.pool2(x_).reshape(B, C, -1).permute(0,2,1)
        r = torch.cat((r1, r2), dim=1) # B N' C
...
class MSCAModule(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., 
                 ...
                 ds_ratio=4):
        super(MSCAModule, self).__init__()
        #
    ...
    def forward(self, s, d):
        ...
        s_ = WH2N(s)
        d_ = WH2N(d)
        s_ = s_ + self.drop_path(self.ms_attn(self.norm1_1(s_), self.norm1_2(d_), H, W))
        s_ = s_ + self.drop_path(self.mlp(self.norm1_3(s_), H, W))
        s_ = N2WH(s_, H, W)
    ...
    def forward(self, inputs):
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            if i == used_backbone_levels - 1:
                laterals[i] = self.fpn_convs[i](laterals[i], laterals[i])
            #
            prev_shape = laterals[i - 1].shape[2:]
            deep_feat = F.interpolate(
                    laterals[i], size=prev_shape, **self.upsample_cfg)
            laterals[i - 1] += self.fpn_convs[i - 1](deep_feat, laterals[i - 1])
